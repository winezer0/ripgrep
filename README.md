...
`gogrep` 是可作为 CLI 使用、也可由外部 Go 工具导入的 PCRE2 文件搜索库。
其目录遍历、ignore 层级和输出结构参考 `go-gogrep`，文本模式全部由
`go-pcre2-lite` 执行。
...
```powershell
$env:GOWORK='off'
go build -buildvcs=false -o gogrep.exe ./cmd/gogrep
./gogrep.exe '(?<=user:)\w+' .
```

支持常用 gogrep 参数，包括递归搜索、ignore、glob、文件类型、上下文、替换、
JSON、颜色、排序、压缩文件和并发线程。退出码与 gogrep 一致：有匹配为 `0`，
无匹配为 `1`，参数或运行错误为 `2`。

当前兼容目标是 `go-gogrep v0.0.5` 已实现的参数集合，并非 Rust gogrep 全部参数。
...
func main() {
	results, err := gogrep.Search(context.Background(), []string{"."}, gogrep.Options{
		Pattern: `(?<=TODO:)\s+\K.+`,
